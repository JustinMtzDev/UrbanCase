...
router.get('/', async (req, res) => {
  try {
    const { rows } = await pool.query(
      'SELECT id, usuario, nombre, rol, activo, created_at FROM usuarios ORDER BY id'
    );
    res.json(rows);
  } catch (err) {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO usuarios (usuario, nombre, password_hash, rol)
       VALUES ($1, $2, $3, $4)
       RETURNING id, usuario, nombre, rol, activo, created_at`,
      [usuario, nombre, hash, rol || 'vendedor']
    );
  ...
  try {
    ...
    const { rows } = await pool.query(
      `UPDATE usuarios SET
       ...
       WHERE id = $6
       RETURNING id, usuario, nombre, rol, activo, created_at`,
      [usuario, nombre, hash, rol, activo, req.params.id]
    );
